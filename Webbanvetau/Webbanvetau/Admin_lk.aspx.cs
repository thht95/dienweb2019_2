...
{
    public partial class Admin_lk : System.Web.UI.Page
    {
        ...
        public void HienLKhach()
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                using (SqlCommand Cmd = new SqlCommand("select * from tblloaikhach", con))
                {
        ...
        protected void grv_sp_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName.ToLower().Equals("xoa"))
            {
                ...
                {
                    using (SqlCommand Cmd1 = new SqlCommand("spLoaikhach_Delete", Cnnxoa))
                        try
                        {
                            Cmd1.CommandType = CommandType.StoredProcedure;
                            Cmd1.Parameters.AddWithValue("@maloaikhach", mak);
                            Cnnxoa.Open();
            ...
            if (e.CommandName == "sua")
            {
                ...
                Label lLk = (Label)row.FindControl("lTenloai");
                Label lGiamgia = (Label)row.FindControl("lPhangiam");
                Label lMaloai = (Label)row.FindControl("lMaloai");

                txtLoaikhach.Text = lLk.Text;
                txtGiamgia.Text = lGiamgia.Text;
        ...
        protected void btnthem_Click(object sender, EventArgs e)
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("spLoaikhach_Insert", cnn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@tenloaikhach", txtLoaikhach.Text);
                    cmd.Parameters.AddWithValue("@phantramgiam", txtGiamgia.Text);
                    cnn.Open();
                    cmd.ExecuteNonQuery();
                }
                cnn.Close();
        ...
        protected void btnsua_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection cnn = new SqlConnection(conString))
            {
                using (SqlCommand cmd = new SqlCommand("spLoaikhach_Update", cnn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add("@maloaikhach", hdtest.Value);
                    cmd.Parameters.AddWithValue("@tenloaikhach", txtLoaikhach.Text);
                    cmd.Parameters.AddWithValue("@phantramgiam", txtGiamgia.Text);
                    cnn.Open();
                    cmd.ExecuteNonQuery();
